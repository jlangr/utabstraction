...
public class PantryTest {
    ...
    public void expieringToday() {
        try {
            ...
            Clock clock = TestClock.fixedTo(d);
            pan.setClock(clock);
            ...
            pan.purchase(item1);
            // add another item
            String item2Name = "blood orange juice 24oz";
            Item item2 = new Item(item2Name);
            item2.setExpirationDate(d.plusDays(1));
            item2.setCategory("orange juice");
            pan.purchase(item2);
            // retrieve
            List<Item> items = pan.getItemsExpiringToday();
            ...
            assertThat("actual items didn't match", actualItemNames, equalTo(expectedItemNames));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
